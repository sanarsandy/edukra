  ...
  <div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
      <div class="flex items-center gap-4">
        <NuxtLink :to="'/instructor/courses'" class="w-10 h-10 flex items-center justify-center bg-neutral-100 rounded-lg hover:bg-neutral-200 transition-colors">
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click="closeModal">
        <div class="bg-white rounded-xl p-6 w-full max-w-lg m-4 max-h-[90vh] overflow-y-auto" @click.stop>
          <div class="flex items-center justify-between mb-6">
            <h2 class="text-xl font-bold text-neutral-900">{{ isEditing ? 'Edit Materi' : (form.is_container ? 'Tambah Modul' : 'Tambah Materi') }}</h2>
          ...
          <form @submit.prevent="saveLesson" class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-neutral-700 mb-2">{{ form.is_container ? 'Nama Modul' : 'Judul Materi' }} <span class="text-red-500">*</span></label>
              <input 
                v-model="form.title"
                type="text" 
                required
                ...
                :placeholder="form.is_container ? 'Contoh: Modul 1 - Pendahuluan' : 'Contoh: Pengenalan HTML'"
              />
            </div>
            ...
            <div v-if="!form.is_container" class="flex items-center gap-3">
            ...
            <div class="flex gap-3 pt-4">
              ...
              <button type="submit" :disabled="saving" class="flex-1 py-2.5 text-sm font-medium text-white bg-admin-600 rounded-lg hover:bg-admin-700 transition-colors disabled:opacity-50">
                {{ saving ? 'Menyimpan...' : (isEditing ? 'Simpan Perubahan' : (form.is_container ? 'Tambah Modul' : 'Tambah Materi')) }}
              </button>
            </div>
          </form>
        </div>
...
useHead({
  title: 'Materi Kursus - Instructor'
})

// API Composables - use instructor panel composable
const instructorPanel = useInstructorPanel()
const { loading, error } = instructorPanel
const lessonsTree = ref<any[]>([])
...
onMounted(async () => {
  const [lessonsResult, courseResult] = await Promise.all([
    instructorPanel.fetchLessonsTree(courseId.value),
    instructorPanel.fetchCourse(courseId.value)
  ])
  if (lessonsResult) lessonsTree.value = lessonsResult.lessons || []
  if (courseResult) course.value = courseResult
})

// Helper to refresh lessons tree
const refreshLessonsTree = async () => {
  const result = await instructorPanel.fetchLessonsTree(courseId.value)
  if (result) lessonsTree.value = result.lessons || []
}
...
const handleCreateQuiz = async () => {
  ...
  const lesson = await instructorPanel.createLesson(courseId.value, lessonData)
  
  if (lesson) {
    // Then create the quiz for this lesson
    const result = await instructorPanel.createQuiz(lesson.id, quizForm.value)
    if (result) {
      showToast('Kuis berhasil dibuat! Tambahkan pertanyaan sekarang.')
      // Keep modal open and switch to questions view
      await refreshLessonsTree()
      // Fetch the quiz to ensure we have the full data
      const quizResult = await instructorPanel.getQuizByLesson(lesson.id)
      if (quizResult) {
        quiz.value = quizResult.quiz
...
const openQuizForLesson = async (lessonItem: any) => {
  ...
  const quizResult = await instructorPanel.getQuizByLesson(lessonItem.id)
...
const handleSaveQuestion = async () => {
  ...
  if (editingQuestion.value) {
    result = await instructorPanel.updateQuestion(editingQuestion.value.id, data)
  } else {
    result = await instructorPanel.addQuestion(quiz.value.id, data)
  }
  ...
  if (result) {
    ...
    if (quiz.value) {
      const quizResult = await instructorPanel.getQuizByLesson(quiz.value.lesson_id)
      if (quizResult) {
        quiz.value = quizResult.quiz
        questions.value = quizResult.questions || []
      }
...
const handleDeleteQuestion = async (q: any) => {
  ...
  const result = await instructorPanel.deleteQuestion(q.id)
  if (result) {
...
const openEditModal = (lesson: any) => {
  ...
  form.value = {
    ...
    is_container: lesson.is_container || false
  }
...
const saveLesson = async () => {
  ...
  try {
    if (isEditing.value && selectedLesson.value) {
      const result = await instructorPanel.updateLesson(selectedLesson.value.id, {
      ...
      const result = await instructorPanel.createLesson(courseId.value, {
        title: form.value.title,
        description: form.value.description,
        content_type: form.value.content_type || undefined,
        video_url: form.value.video_url || undefined,
...
const confirmDeleteLesson = async () => {
  ...
  saving.value = true
  try {
    const success = await instructorPanel.deleteLesson(selectedLesson.value.id)
    if (success) {
...
const uploadFile = async (file: File) => {
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/api/instructor/upload`, {
      method: 'POST',
      headers: {
